...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    L1 = torch.mm(X,W1)+b1
    hidden = torch.maximum(L1,torch.zeros_like(L1))
...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    x_idx = torch.arange(N)
    y_idx = y

    scores_exp = torch.exp(scores)
    loss_per= -torch.log(scores_exp[x_idx,y_idx]/torch.sum(scores_exp,dim=-1))
    loss = torch.sum(loss_per)/N+torch.sum(W2*W2)*reg
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    ###对于db的梯度，由于b使用相同行初始化一行，所以不能直接用dS计算，维度不对，
    ###要用求和降低维度，简单理解，也就是db也是要凑维度和b一样，但是多变少，直接加

    dS = torch.zeros_like(scores_exp)
    dS = scores_exp/torch.sum(scores_exp,dim=-1,keepdim=True)
    dS[x_idx,y_idx] = (-1+scores_exp[x_idx,y_idx]/torch.sum(scores_exp,dim=-1))
    grads['W2'] = torch.mm(h1.t(),(dS/N))+2*W2*reg
    grads['b2'] = torch.sum(dS/N,dim=-2)

    dH1 = torch.mm(dS/N,W2.t())
    dL1 = dH1*(h1>0)
    
    grads['W1'] = torch.mm(X.t(),dL1)
    grads['b1'] = torch.sum(dL1,dim=-2)
    
    ###重要经验，relu的导数，也就是h1>0,不能用dh1，要用自己输入的矩阵h1计算relu导数
        ...
        for regularization_strength in regularization_strengths:
          for learning_rate_decay in learning_rate_decays:
            ...
            stats = net.train(data_dict['X_train'],data_dict['y_train'],data_dict['X_val'],data_dict['y_val'],\
                  learning_rate,learning_rate_decay,regularization_strength,num_iters=1000,batch_size=500,\
                  verbose=False)
            y_val_preds = net.predict(data_dict['X_val'])
            val_acc_now = (y_val_preds == data_dict['y_val']).double().mean().item()
            if val_acc_now > best_val_acc:
              best_net, best_stat, best_val_acc=net,stats,val_acc_now
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
