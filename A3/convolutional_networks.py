...
class Conv(object):
          ...
          for f in range(F):
            for h in range(H_out):
              for width in range(W_out):
                out[n,f,h,width] = torch.sum(x_pad[n,:,h*stride:h*stride+HH,width*stride:width*stride+WW]*w[f])+b[f]
    ...
    def backward(dout, cache):
        ...
        ##经验：！注意，由于卷积运算是一个卷积核和输入矩阵一部分进行运算，生成一个标量
        ##因此反向求导，就是循环每个标量来求对应的原来计算的小矩阵的元素的导数

        ###注意计算关系，输入x,w，其中x的后三个维度和w的后三个维度一次运算得到一个标量
        ###w[i]代表第i个卷积核，用来和x的小矩阵运算生成一个标量，每次更新w也是更新w[i]
        ###而x的计算，则是每次一个图片运算，所有channel都要进行运算
        ###所以，一般正反向求导，都是相同的图片个数n，然后x使用所有通道，w选择第几个核（对应通道号），
        ###主要操作h和w的范围，在x中要乘上stride和索引对应范围
        for n in range(N):
          for f in range(F):
            for h in range(H_out):
              for width in range(W_out):
                dx[n,:,h*stride:h*stride+HH,width*stride:width*stride+WW] += dout[n,f,h,width]*w[f]
                dw[f] += dout[n,f,h,width]*x_pad[n,:,h*stride:h*stride+HH,width*stride:width*stride+WW]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        for n in range(N):
          for c in range(C):
            for h in range(H_out):
              for width in range(W_out):
                 out[n,c,h,width] = torch.max(x[n,c,h*stride:h*stride+pool_height,width*stride:width*stride+pool_width])
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        ###经验！！！！：无论是反传播还是池化反向，都是将一个结果标量传播到对应计算的小矩阵和卷积核，因此直接分析乘
        ####！！！！！！！！！！！！！！
        ####！！！关于stride，是否坐标乘上，只需要一件事，就是先看输出结果的坐标，然后反向用输出坐标乘stride得到x的内部的实际坐标
        ####因为输出的小矩阵必然是已经stride压缩的，只要以他的坐标为基础，乘上stride得到原大矩阵x的实际坐标即可
        ...
        #经验：构造张量务必注意dtype和device
        for n in range(N):
          for c in range(C):
            for h in range(H_out):
              for width in range(W_out):
                pool_matrix = x[n,c,h*stride:h*stride+pool_height,width*stride:width*stride+pool_width]
                index_matrix = pool_matrix == torch.max(pool_matrix)
                ##经验：用mask索引赋值梯度的时候，不要忘了先切片出对应的小矩阵
                dx[n,c,h*stride:h*stride+pool_height,width*stride:width*stride+pool_width][index_matrix] = dout[n,c,h,width]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ###经验：卷积-relu-pool总共会有两次改变大小的机会，一次是conv，一次是pool，要两次计算新的结果矩阵形式
    ...
    def loss(self, X, y=None):
        ...
        H3,cache3=Linear.forward(H2,W3,b3)
        scores = H3
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dh2,grads['W3'],grads['b3']=Linear.backward(ds,cache3)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        if weight_scale == 'kaiming':
          for num_layer in range(self.num_layers-1):
            F = num_filters[num_layer]
            self.params['W'+str(num_layer+1)] = kaiming_initializer(C, F, K=3, relu=True, device=device,dtype=dtype)
            self.params['b'+str(num_layer+1)] = torch.zeros(F,device=device,dtype=dtype)
            C = F
            if self.batchnorm:
              self.params[f'gamma{num_layer+1}'] = torch.ones(F,device=device,dtype=dtype)
              self.params[f'beta{num_layer+1}'] = torch.zeros(F,device=device,dtype=dtype)
          self.params['W'+str(self.num_layers)] = kaiming_initializer(num_filters[-1]*(H//(2**(len(max_pools))))*(W//(2**(len(max_pools)))),num_classes, relu=False, device=device,dtype=dtype)
          self.params['b'+str(self.num_layers)] = torch.zeros(num_classes,dtype=dtype,device=device)
        else:
          for num_layer in range(self.num_layers-1):
            F = num_filters[num_layer]
            self.params['W'+str(num_layer+1)] = torch.randn(F,C,3,3,device=device,dtype=dtype)*weight_scale
            self.params['b'+str(num_layer+1)] = torch.zeros(F,device=device,dtype=dtype)
            C = F
            if self.batchnorm:
              self.params[f'gamma{num_layer+1}'] = torch.ones(F,device=device,dtype=dtype)
              self.params[f'beta{num_layer+1}'] = torch.zeros(F,device=device,dtype=dtype)
          self.params['W'+str(self.num_layers)] = torch.randn(num_filters[-1]*(H//(2**(len(max_pools))))*(W//(2**(len(max_pools)))),num_classes,device=device,dtype=dtype)*weight_scale
          self.params['b'+str(self.num_layers)] = torch.zeros(num_classes,dtype=dtype,device=device)
    ...
    def loss(self, X, y=None):
        ...
        input_m = X
        for num_layer in range(self.num_layers-1):
          w = self.params[f'W{num_layer+1}']
          b = self.params[f'b{num_layer+1}']
          if self.batchnorm:
            if num_layer in self.max_pools:
              gamma = self.params[f'gamma{num_layer+1}']
              beta = self.params[f'beta{num_layer+1}']
              input_m,cache[f'cache{num_layer+1}'] = Conv_BatchNorm_ReLU_Pool.forward(input_m,w,b,gamma,beta,conv_param,self.bn_params[num_layer],pool_param)
            else:
              input_m,cache[f'cache{num_layer+1}'] = Conv_BatchNorm_ReLU.forward(input_m,w,b,gamma,beta,conv_param,self.bn_params[num_layer])
          else:
            if num_layer in self.max_pools:
              input_m,cache[f'cache{num_layer+1}'] = Conv_ReLU_Pool.forward(input_m,w,b,conv_param,pool_param)
            else:
              input_m,cache[f'cache{num_layer+1}'] = Conv_ReLU.forward(input_m,w,b,conv_param)
        w = self.params[f'W{self.num_layers}']
        b = self.params[f'b{self.num_layers}']
        scores,cache[f'cache{self.num_layers}'] = Linear.forward(input_m,w,b)
        ...
        for num_layer in range(self.num_layers):
          loss += self.reg*torch.sum(self.params[f'W{num_layer+1}']**2)

        dh,grads[f'W{self.num_layers}'],grads[f'b{self.num_layers}'] = Linear.backward(ds,cache[f'cache{self.num_layers}'])
        grads[f'W{self.num_layers}'] += 2*self.reg*self.params[f'W{self.num_layers}']

        for num_layer in reversed(range(self.num_layers-1)):
          if self.batchnorm:
            if num_layer in self.max_pools:
              dh,grads[f'W{num_layer+1}'],grads[f'b{num_layer+1}']=Conv_BatchNorm_ReLU_Pool.backward(dh,cache[f'cache{num_layer+1}'])
            else:
              dh,grads[f'W{num_layer+1}'],grads[f'b{num_layer+1}']=Conv_BatchNorm_ReLU.backward(dh,cache[f'cache{num_layer+1}'])
          else:
            if num_layer in self.max_pools:
              dh,grads[f'W{num_layer+1}'],grads[f'b{num_layer+1}']=Conv_ReLU_Pool.backward(dh,cache[f'cache{num_layer+1}'])
            else:
              dh,grads[f'W{num_layer+1}'],grads[f'b{num_layer+1}']=Conv_ReLU.backward(dh,cache[f'cache{num_layer+1}'])
          grads[f'W{num_layer+1}'] += 2*self.reg*self.params[f'W{num_layer+1}']
