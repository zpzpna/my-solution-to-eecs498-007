...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        x_flatten = x.reshape(num_trains,-1)
        out = torch.mm(x_flatten,w)+b.view(1,-1)
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        ###经验：判断upgrad形状，看当前x和w计算后矩阵的形状，因为某一层梯度矩阵和结果矩阵形状相同
        num_trains = x.shape[0]
        x_flatten = x.reshape(num_trains,-1)

        #先flatten方便计算梯度后，还需要将dx变成原来的形状
        dx = torch.mm(dout,w.t()).view(x.shape)
        dw = torch.mm(x_flatten.t(),dout)
        db = torch.sum(dout,dim=-2)
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ##记住初始化操作！！！搭一个模型，首先将线性层和激活层的正反传播类写完，注意静态方法，输入具体矩阵
        ##然后在模型类中，初始化输入相关层数进行参数初始化！，然后设计loss和grad


        ##初始化时务必设置device和dtype！！！！！！建模型时候设置
        ...
        self.params['b1'] = torch.zeros(1,hidden_dim,dtype=dtype,device=device)
        self.params['b2'] = torch.zeros(1,num_classes,dtype=dtype,device=device)
        ###############################################################
        #                            END OF YOUR CODE                 #
        ###############################################################
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        hiddens, cache1 = Linear_ReLU.forward(X,self.params['W1'],self.params['b1'])
        scores, cache2 = Linear.forward(hiddens,self.params['W2'],self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        #无论是初始化还是grad保存参数，都要用字典，loss在反向传播中完成？
        ##这里从loss标量到ds在loss函数中完成
        ...
        dX,grads['W1'],grads['b1']= Linear_ReLU.backward(dH,cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ###最后的综合模型算loss类，需要明确指定device和dtype，所有参数这两个属性相同


        hidden_num = 0
        d_in ,d_out = input_dim,hidden_dims[hidden_num]
        self.params['W'+str(hidden_num+1)] = torch.randn((d_in,d_out),dtype=dtype,device=device)*weight_scale
        self.params['b'+str(hidden_num+1)] = torch.zeros((1,d_out),dtype=dtype,device=device)
        hidden_num += 1
        d_in = d_out

        for d_out in hidden_dims[1:]:
          self.params['W'+str(hidden_num+1)] = torch.randn((d_in,d_out),dtype=dtype,device=device)*weight_scale
          self.params['b'+str(hidden_num+1)] = torch.zeros((1,d_out),dtype=dtype,device=device)
          hidden_num += 1
          d_in = d_out
    ...
    def loss(self, X, y=None):
        ...
        ###经验：能用range一定不要用arange出现tensor类型错误，尤其是字典索
        for i in range(self.num_layers-1):
          curr_mat, cache[str(i+1)]= Linear_ReLU.forward(curr_mat,self.params['W'+str(i+1)],self.params['b'+str(i+1)])
        scores ,cache[str(self.num_layers)]= Linear.forward(curr_mat,self.params['W'+str(self.num_layers)],self.params['b'+str(self.num_layers)])
        ...
        dH,grads['W'+str(self.num_layers)],grads['b'+str(self.num_layers)] = Linear.backward(ds,cache[str(self.num_layers)])
        grads['W'+str(self.num_layers)] += 2*self.reg*self.params['W'+str(self.num_layers)]
        for i in reversed(range(self.num_layers-1)):
          dH,grads['W'+str(i+1)],grads['b'+str(i+1)] = Linear_ReLU.backward(dH,cache[str(i+1)])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model,data_dict,device=device,update_rule=sgd,num_epochs=100)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def get_three_layer_network_params():
    ...
    # Replace "pass" statement with your code
    weight_scale = 4.9999e-2
    learning_rate = 0.99999e0
    ################################################################
    #                             END OF YOUR CODE                 #
    ################################################################
...
def sgd_momentum(w, dw, config=None):
    ...
    ##################################################################
    # Replace "pass" statement with your code
    #v = config['momentum']*v+dw*(1-config['momentum'])
...
def rmsprop(w, dw, config=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    grad_squared = config['cache']*config['decay_rate']+(1-config['decay_rate'])*dw*dw
    next_w = w - config['learning_rate']*dw/(grad_squared+config['epsilon']).sqrt()
...
def adam(w, dw, config=None):
    ...
    ##经验：对于sgd+momentum,一般单独使用时，对动量v更新学习率，综合使用时，计算移动平均
    ##经验！！：对于Adam注意计算更新梯度用的是moment_unbias,保存的是moment
    moment1 = config['beta1']*config['m']+(1-config['beta1'])*dw
    moment2 = config['beta2']*config['v']+(1-config['beta2'])*dw*dw
    moment1_unbias =moment1/ (1-config['beta1']**config['t'])
    moment2_unbias =moment2/ (1-config['beta2']**config['t'])
    config['m'] = moment1
    config['v'] = moment2
