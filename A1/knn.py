...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for test_idx in range(num_test):
      for train_idx in range(num_train):
        dist = ((x_train_flatten[train_idx,:] - x_test_flatten[test_idx,:])**2).sum().sqrt()
        dists[train_idx,test_idx] = dist
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flatten = x_test.view(num_test,-1)
    for test_idx in range(num_test):
      dist = ((x_train_flatten-x_test_flatten[test_idx,:])**2).sum(dim=-1,keepdim=True).sqrt()
      #注意索引要用保留矩阵格式的索引方法
      dists[:,test_idx:test_idx+1] = dist
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    #x_train_vec = x_train_flatten.reshape(torch.Size([1])+x_train_flatten.shape).transpose(0,1)
    #x_test_vec = x_test_flatten.reshape(torch.Size([1])+x_test_flatten.shape)
    #dists = ((x_train_vec - x_test_vec)**2).sum(dim=-1).sqrt()
    #因为我是用test转换维度来匹配，所以最后变成（test，train）需要交换维度
    #####！！！！！！！这种三维广播计算很慢，因为他要广播成大矩阵，尽量数学公式转化二维计算

    x_train_pow = (x_train_flatten**2).sum(dim=-1,keepdim=True)
    x_test_pow = (x_test_flatten**2).sum(dim=-1,keepdim=True)
    xy = x_train_flatten.mm(x_test_flatten.t())
    dists = (x_train_pow+x_test_pow.t()-xy*2).sqrt()
    ####!!!注意，对于这种多项式拆分后的结果，要搞懂最终目标位置和拆下来的每一项关系
    #！！！！把拆下来类似的项放一块计算
    #比如平方差中，x_train_pow一行代表一张train图片和test图片算距离时的所有x1**2之和
    #总结：将原来的平方差公式展开成新的若干项分别计算后求和即可
    #注意：比起语义，当成向量组合求每个项更容易理解。！！！！！！！！！
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ###knn预测原理：
    #！！首先选择k个最近邻，然后预测时，计算test图和train中图的l2距离，找到k个最近的图
    #找到后，根据k个图对应的种类，统计每个种类（标签）出现次数，选最大数目的种类为预测类
    #当有多个种类出现次数最多且相同，这就出现tie的竞争情况
    ###eg:k=5,预测一个test图，找到5个距离最近的train图，统计对应种类：猫：3，鼠：1，狗：1，预测test图为猫
    idx = torch.topk(dists,k,dim=-2,largest = False).indices
    for i in range(num_test):
      y_pred[i] = torch.mode(y_train[idx[:,i]]).values

    ##经验，官方文档的返回模式每个属性，可以用.在最后直接获取单个属性
    ###mode输入张量和dim，获取对应dim下的众数和位置，注意，获取的众数张量形状
    #是上一个维度方向形状

    ###topk要选择largest是否是最大几个还是最小几个，也要选择dim和k
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        x_train_cat = torch.cat(x_train_folds[:i]+x_train_folds[i+1:])
        y_train_cat = torch.cat(y_train_folds[:i]+y_train_folds[i+1:])
        model = KnnClassifier(x_train_cat,y_train_cat)
        k_to_accuracies[k].append(model.check_accuracy(x_train_folds[i],y_train_folds[i],k,False))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k,v in k_to_accuracies.items():
      mean_now = sum(v)/len(v)
      if mean_best<mean_now:
        mean_best = mean_now
        best_k = k
    ##########################################################################
    #                           END OF YOUR CODE                             #
